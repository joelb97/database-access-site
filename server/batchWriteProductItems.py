...
def lambda_handler(event, context):
    ...
    for table, _items in res.items():
        if 'Product' in _items:
            for item in _items['Product']:
...
def verify_input(event):
    ...
    for item in event['items']:
        ...
        if 'Zip' not in item or 'Recorded' not in item or 'Product Info' not in item or 'Modified User' not in item or 'ORG User' not in item:
            raise Exception(str(item) + " is not the correct format.")
    if not event['items']:
    ...
    for item in event['items']:
        for key in item:
            if item[key] == 'N/A' and item['Modified User'] != "":  # if modified user is not specified, item is ignored
                raise Exception(
                    'Object containing ' + str(key) + ': N/A is not allowed to be inserted into the database.')